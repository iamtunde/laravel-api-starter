...
class LoadController extends Controller
{
    ...
    public function loginUser(Request $request)
    {
        try {
            ...
            if($validation->fails())
            {
                ...
                $errorMessages = $this->formatErrors($validation->getMessageBag()->messages());
                ...
                if($findUser !== null && $findUser->isDeleted == false)
                {
                    ...
                    if(!$token = $this->auth::attempt($data))
                    {
                        ...
                        return response()->json(["message" =>  "Incorrect login details", "data" => $data, "error" => true], 401);
                    } else {
                        ...
                        return response()->json(['message' => "Login successfull", "data" => $user, "error" => false], 200);
                    }
                } else {
                    //user account was deleted
                    return response()->json(["message" =>  "User account not found, create an account today", "data" => $data, "error" => true], 401);
                }
            }
        } catch(\Exception $e) {
    ...
    public function sendPasswordRecoveryMail(Request $request)
    {
        try {
            ...
            if($validation->fails()) {
                ...
                if($findEmail !== null)
                {
                    ...
                    //email not found bro
                    return response()->json(["message" => "User not found for provided email", "data" => $body], 404);
                }
            }
        } catch(\Exception $e) {
        ...
        return response()->json(["message" => "Password recovery link was sent sucessfully", "data" => $body], 200);
    }
    ...
    public function resetPassword(Request $request)
    {
        try {
            ...
            $validation = $this->validateInput($body, "reset-password");
        ...
        return response()->json(["message" => "Password successfully changed", "data" => $body, "error" => false], 200);
    }
}